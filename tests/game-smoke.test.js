...
vm.runInContext(`${fs.readFileSync(path.join(root, 'game.js'), 'utf8')}
  ...
  const manualTower = towerFactory.create({ col: 1, row: 6, level: 1 });
  const manualPosition = center(manualTower);
  const manualObject = { x: manualPosition.x + 40, y: manualPosition.y, hp: 120, max: 120, dead: false, cleared: false };
  towers = [manualTower]; enemies = []; mapObjects = [manualObject]; started = true; running = true;
  const automaticTargets = towerCombatContext().enemies;
  manualTower.manualTarget = manualObject;
  const manualContext = towerCombatContext();
  const manualSelection = attackPatterns.get(evolution.base.attackMode).select(manualTower, manualContext.enemies, evolution.base, manualContext);
  globalThis.__smoke.mapObjectsRequireManualOrder = !automaticTargets.includes(manualObject) && manualSelection[0] === manualObject;
  manualObject.dead = true;
  towerCombatContext();
  globalThis.__smoke.manualOrderClearsWithTarget = !manualTower.manualTarget;
...
assert.equal(sandbox.__smoke.mapObjectsRequireManualOrder, true);
assert.equal(sandbox.__smoke.manualOrderClearsWithTarget, true);
